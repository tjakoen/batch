...
[![npm](https://img.shields.io/npm/v/@tjakoen/batch?logo=npm&logoColor=white)](https://www.npmjs.com/package/@tjakoen/batch)
...
and the audit engine. (The component catalog moved up to GRAIN, `grain/packages/grain/catalog`.) The app that
...
- **Also:** the composition engine (zero runtime deps) · a generic SSE push hub · sitemap/SEO from
  ...
  moved up to GRAIN, `grain/packages/grain/catalog`.)
...
- **One folder per component**: `<components-root>/<level>/<name>/<name>.{html,css}` (atoms → molecules → organisms), CSS co-located with its template. The roots are wired by the consumer (`tjakoen.github.io/src/config.ts`, which points at the installed `@tjakoen/grain` package's `components` dir and at its own `view/components`).
- **Flat-file pages**: `<pages-root>/about.html` → `/about` (here: `tjakoen.github.io/view/pages`); folders only group subpages. URL mirrors the tree. Minimal JS sits in a `<script>` after the UI.
...
- **Tokens live up in GRAIN** (`grain/packages/grain/styles`). BATCH ships the *mechanism*, not a theme: it bundles each component's co-located CSS into `/components.css` at request time, no build step.
...
- **One write path**: all mutation flows through GRAIN's single door (`POST /intent` → render ops over SSE), not a separate CRUD API. BATCH just provides the generic SSE hub; the vocabulary lives in `grain/packages/grain/ai/*`.
