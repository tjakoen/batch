// batch/catalog/catalog.test.ts — the catalog page shell honors the injection seams.
...
describe("createCatalog inject seams", () => {
  ...
  test("headEnd lands in <head>, bodyEnd before </body>", async () => {
    const catalog = createCatalog(bunRuntime, dir, undefined,
  ...
  test("no inject → shell renders without placeholders", async () => {
    const catalog = createCatalog(bunRuntime, dir);
    const html = await catalog.html();
